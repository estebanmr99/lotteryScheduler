...
void updateSum(Function *function){
    ...
    if(GTK_IS_PROGRESS_BAR(progressBar)){
        ...
        gchar *buffer = g_strdup_printf("Progress: %.0f%%  Sum: %0.20Le",fraction*100,function->sum);
        if(fraction< 1.0){
            if(label!=NULL){
            ...
            if(label!=NULL){
                gchar *buffer1 = g_strdup_printf("DONE  Sum: %0.20Le",function->sum);
...
void readConsole(){
    ...
    for(int i = 0; i < T; i++)
    {
        ...
        functions[i].bar->label = gtk_label_new("Waiting...0%");
...
void napierian(void* p){
    ...
    for(n = 1; n <= f -> M; n++,i++){
        ...
        if(i==f->n){
            ...
            gtk_button_clicked(GTK_BUTTON(f->bar->progress));
        }    
...
void euler(void* p){ // EL factorial no debe de calcularse a mas de 1754
    ...
    for (n = 0; n <= f -> M; n++,i++) {
        ...
        if(i==f->n){
            gtk_button_clicked(GTK_BUTTON(f->bar->widget));
            i=0;
            //printf("AUMENTA %% EULER");
        }

        else{
            gtk_button_clicked(GTK_BUTTON(f->bar->progress));
            //printf("NO AUMENTA %% EULER");
        }    
        yield(f->c);
    }
}
